...
#MYSQL Configuration
MYSQL_CONFIG= {
    "url":"jdbc:mysql://34.66.98.139:3306/hospital_a_db?useSSL=true&requireSSL=true&allowPublicKeyRetrieval=true&verifyServerCertificate=false",
...
log_entries = [] #Stores log before writting it to GCS
...
def get_latest_watermark(table_name):
    query = f"""
    SELECT MAX(LOAD_TIMESTAMP) AS latest_timestamp FROM `{BQ_AUDIT_TABLE}` WHERE table_name = '{table_name}' AND data_source = 'hospital_a_db'
    """
...
def extract_and_save_to_landing(table, load_type, watermark_col):
    try:
        ...
        #Insert Audit Entry
        audit_df = spark.createDataFrame([("hospital_a_db", table, load_type,df.count(), datetime.datetime.now(), "SUCCESS")],
